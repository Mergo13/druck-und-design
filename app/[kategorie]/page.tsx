...
export function generateStaticParams() {
  return platformCategories.map((category) => ({ kategorie: category.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ kategorie: string }> }): Promise<Metadata> {
  const { kategorie } = await params;
  const category = platformCategories.find((item) => item.slug === kategorie);
  return {
...
export default async function CategoryPage({ params }: { params: Promise<{ kategorie: string }> }) {
  ...
  return (
    <section className="container-page py-10">
      ...
      <ShopBrowser initialCategory={category.slug} />
    </section>
  );
}
