...
export function Footer() {
  return (
    <footer className="mt-20 border-t border-slate-200 bg-[linear-gradient(180deg,#f8fbff,#f2f7ff)] text-slate-800">
          ...
          {cols.map((col) => (
            <div key={col[0].label}>
              ...
              <div className="mt-4 grid gap-2">
                {col.slice(1).map((item) => <Link className="text-sm text-slate-600 transition hover:text-brand-blue" href={item.href} key={`${item.label}-${item.href}`}>{item.label}</Link>)}
              </div>
            </div>
          ))}
        </div>
