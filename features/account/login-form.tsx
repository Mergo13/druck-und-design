...
export function LoginForm() {
  ...
  const isAdminLogin = requestedPath?.startsWith("/admin") ?? false;
  const isForbidden = searchParams.get("error") === "forbidden";
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [message, setMessage] = useState(isForbidden ? "Kein aktiver Admin-Zugang für diese E-Mail." : "");
  ...
  return (
    <form onSubmit={(event) => void onSubmit(event)} className="mx-auto w-full max-w-md rounded-lg border bg-white p-6 shadow-premium">
      <h1 className="text-3xl font-black">{isAdminLogin ? "Admin Login" : "Einloggen"}</h1>
      <p className="mt-2 text-sm text-muted-foreground">
        {isAdminLogin
          ? "Melden Sie sich mit einem aktiven Admin-Konto an."
          : "Greifen Sie auf Warenkorb und Angebotsanfragen zu."}
      </p>
      ...
      <Button className="mt-6 w-full" disabled={loading}>{loading ? "Bitte warten..." : isAdminLogin ? "Als Admin einloggen" : "Einloggen"}</Button>
